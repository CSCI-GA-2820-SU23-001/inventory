...
class TestYourResourceServerCreate(TestResourceServer):
    ...
    def test_create_with_invalid_numbers(self):
        ...
        test_inventory = InventoryFactory(product_id=1, condition=Condition.NEW, quantity=-2)
        response = self.client.post(BASE_URL, json=test_inventory.serialize())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        test_inventory = InventoryFactory(product_id=1, condition=Condition.NEW, quantity=2, restock_level=-3)
        response = self.client.post(BASE_URL, json=test_inventory.serialize())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        test_inventory = InventoryFactory(product_id=1, condition=Condition.NEW, quantity=2, restock_level=0)
        response = self.client.post(BASE_URL, json=test_inventory.serialize())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        test_inventory = InventoryFactory(product_id=1, condition=Condition.NEW, quantity=0, restock_level=3)
        response = self.client.post(BASE_URL, json=test_inventory.serialize())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        test_inventory = InventoryFactory(product_id=0, condition=Condition.NEW, quantity=2, restock_level=3)
        response = self.client.post(BASE_URL, json=test_inventory.serialize())
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
